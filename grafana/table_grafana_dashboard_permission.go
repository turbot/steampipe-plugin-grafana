...
func tableGrafanaDashboardPermission(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		List: &plugin.ListConfig{
			ParentHydrate: listDashboard,
			KeyColumns:    plugin.OptionalColumns([]string{"dashboard_uid"}),
			Hydrate:       listDashboardPermission,
		},
		Columns: []*plugin.Column{
			// Top columns
			{Name: "dashboard_uid", Type: proto.ColumnType_STRING, Description: "Globally unique identifier for the dashboard."},
			...
			{Name: "is_folder", Type: proto.ColumnType_BOOL, Description: "True if the permission applies to a folder rather than a dashboard."},
...
func listDashboardPermission(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
