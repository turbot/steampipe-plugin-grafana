...
func tableGrafanaFolderPermission(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		List: &plugin.ListConfig{
			ParentHydrate: listFolder,
			KeyColumns:    plugin.OptionalColumns([]string{"folder_uid"}),
			Hydrate:       listFolderPermission,
		},
		Columns: []*plugin.Column{
			// Top columns
			{Name: "folder_uid", Type: proto.ColumnType_STRING, Description: "Globally unique identifier for the folder."},
...
func listFolderPermission(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	conn, err := connect(ctx, d)
	if err != nil {
		plugin.Logger(ctx).Error("grafana_folder_permission.listFolderPermission", "connection_error", err)
		return nil, err
	}

	folder := h.Item.(*models.FolderSearchHit)
	fuid := folder.UID
